...
class Environment():
  ...
  def render_bot(self):
    ...
    self.canvas.create_line(self.bot.x+body_x1, self.bot.y+body_y1, self.bot.x+body_x2, self.bot.y+body_y2, fill="blue", tags="bot")

    #Draw head to indicate direction bot is facing
    head_x2, head_y2 = polar_to_cartesian(RENDER_HEAD_RADIUS, self.bot.theta)
    self.canvas.create_line(self.bot.x, self.bot.y, self.bot.x+head_x2, self.bot.y+head_y2, fill="red", tags="bot")

    #Draw single dot to track where we've been
    if self.t%TRACKER_DOT_STEP==0:
      self.canvas.create_line(self.bot.x, self.bot.y, self.bot.x, self.bot.y+1, fill="green")
    self.t+=1

  def spawn_bot(self, event):
    #Give it a cage to use for sonar
    self.canvas.create_rectangle(self.bot.x-CANVAS_CAGE_WIDTH, self.bot.y-CANVAS_CAGE_HEIGHT, self.bot.x+CANVAS_CAGE_WIDTH, self.bot.y+CANVAS_CAGE_HEIGHT, outline="black")
